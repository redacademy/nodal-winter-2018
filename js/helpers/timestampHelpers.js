...
export const formatCompetitionTime = (start, end) =>{
  if (start.getDate() !== end.getDate()){
    return `${getDayOfWeek(start)}, ${getMonthOfYear(start)} ${start.getDate()} ${formatHours(start)}, ${start.getFullYear()} -\n${getDayOfWeek(end)}, ${getMonthOfYear(end)} ${end.getDate()} ${formatHours(end)}, ${end.getFullYear()}`
  } else {
    return `${getDayOfWeek(start)}, ${getMonthOfYear(start)} ${start.getDate()}, ${start.getFullYear()}\n ${formatHours(start)} -${formatHours(end)}`
  }
}
